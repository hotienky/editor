...
export class LayoutEngine {
  ...
  compute(nodes, pageOptions = {}) {
    ...
    // Build page numbers with section configs
    const sectionConfigs = pageAssignments
      .filter(p => p.config && p.config !== config)
      .map((p, i) => ({ pageNumber: p.pageNumber, ...p.config }))
    const pageNums = buildPageNumbers(totalPages, pageOptions.pageNumbers || {}, sectionConfigs)
  ...
  _buildLayoutPages(nodes, blocks, pageAssignments, contentArea, hfMap, pageNums) {
    ...
    for (const assignment of pageAssignments) {
      ...
      const sectionConfig = assignment.config || {}
