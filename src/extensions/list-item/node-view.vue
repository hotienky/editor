<template>
  ...
  >
    <t-dropdown
      v-if="isOrderedList"
      trigger="click"
      overlay-class-name="kindy-list-item-popup"
      :visible="editor?.isEditable && markerMenuVisible"
      size="small"
      :popup-props="popupProps"
    >
      <span
        class="kindy-list-item-marker is-ordered-list-marker"
        contenteditable="false"
        data-list-marker=""
        @mousedown.prevent
        @click.stop="focusListItem"
      >
        <span class="kindy-list-item-marker-text">{{ markerText }}</span>
      </span>
      <template #dropdown>
        <t-dropdown-menu>
          <t-dropdown-item
            class="kindy-list-item-menu-item"
          ...
          <t-dropdown-item class="kindy-list-item-menu-item">
            <t-dropdown
              class="kindy-list-item-menu-item"
              trigger="click"
              placement="right-top"
            >
              <div class="kindy-list-item-submenu-trigger" @click.stop>
    ...
    >
      <span
        class="kindy-list-item-marker is-bullet-list-marker"
        contenteditable="false"
        data-list-marker=""
        @mousedown.prevent
        @click.stop="focusListItem"
    ...
    <label
      ...
      class="kindy-list-item-task-marker"
      contenteditable="false"
      @mousedown.prevent
    >
      <input
        class="kindy-list-item-task-checkbox"
        type="checkbox"
    ...
    >
      <div class="kindy-list-item-start-dialog">
        <t-input-number
...
const clearMarkerMetricVars = (wrapperElement) => {
  ...
  wrapperElement.style.removeProperty('--kindy-list-marker-font-size')
  wrapperElement.style.removeProperty('--kindy-list-marker-offset-y')
}
...
const syncMarkerMetrics = () => {
  ...
  wrapperElement.style.setProperty(
    '--kindy-list-marker-font-size',
    Number.isFinite(parsedFontSize) && parsedFontSize > 0
      ? `${parsedFontSize}px`
      : fontSize,
  )
  wrapperElement.style.setProperty(
    '--kindy-list-marker-offset-y',
    `${markerOffsetY}px`,
  )
}
...
const popupProps = $computed(() => ({
  ...
  overlayClassName: 'kindy-list-item-overlay',
...
.kindy-list-item-popup {
  .kindy-popup__content {
    min-width: 180px;
  }
}

.kindy-list-item-menu-item {
...
.kindy-list-item-submenu-marker {
  display: inline-flex;
  align-items: center;
  font-size: 18px;
  line-height: 1;
...
.kindy-list-item {
  --offset-y: var(--kindy-list-marker-offset-y, 0);
  --font-size: var(--kindy-list-marker-font-size, inherit);
  display: flex;
  ...
  .kindy-list-item-marker-text {
    display: inline-block;
    line-height: 1;
    white-space: nowrap;
  }

  .kindy-list-item-task-marker {
    display: inline-flex;
    align-items: center;
    justify-content: center;
    transform: translateY(var(--offset-y));
