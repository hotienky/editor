...
import { deepClone, nextTick, pt2px } from '../../utils'
...
export class Draw {
  ...
  public getElementFont(el: IElement, scale = 1): string {
    const { defaultSize, defaultFont, sizeUnit } = this.options
    const font = el.font || defaultFont
    const size = el.actualSize || el.size || defaultSize
    const pxSize = sizeUnit === 'px' ? size : pt2px(size)
  ...
  public getElementSize(el: IElement) {
    const size = el.actualSize || el.size || this.options.defaultSize
    return this.options.sizeUnit === 'px' ? size : pt2px(size)
  }
  ...
  public getElementRowMargin(el: IElement) {
    ...
    const marginMultiplier = el.rowMargin ?? defaultRowMargin
    if (marginMultiplier <= 0) return 0
    if (el.type === ElementType.TABLE) {
      return defaultBasicRowMarginHeight * marginMultiplier * scale
    }
    const fontSize = this.getElementSize(el)
    let marginRate: number
    if (marginMultiplier <= 1) {
      marginRate = marginMultiplier * 0.1
    } else {
      marginRate = 0.1 + (marginMultiplier - 1) * 0.6
    }
    return marginRate * fontSize * scale
  }
  ...
  public computeRowList(payload: IComputeRowListPayload) {
    ...
    for (let i = 0; i < elementList.length; i++) {
      ...
      if (
        ...
        const fontAscent =
          basisMetrics.fontBoundingBoxAscent ||
          basisMetrics.actualBoundingBoxAscent ||
          defaultSize * 0.8
        const fontDescent =
          basisMetrics.fontBoundingBoxDescent ||
          basisMetrics.actualBoundingBoxDescent ||
          defaultSize * 0.2
        metrics.boundingBoxAscent = fontAscent * scale
        metrics.boundingBoxDescent = fontDescent * scale
        ...
        metrics.height = this.getElementSize(element) * scale
        metrics.boundingBoxDescent = 0
        metrics.boundingBoxAscent =
        ...
        const elSize = this.getElementSize(element)
        metrics.height = elSize * scale
        ctx.font = this.getElementFont(element)
        const fontMetrics = this.textParticle.measureText(ctx, element)
        ...
        if (
          ...
          if (this.WORD_LIKE_REG.test(word)) {
            ...
            if (wordWidth <= availableWidth) {
              curRowWidth = curRow.width + wordWidth
              if (endElement) {
