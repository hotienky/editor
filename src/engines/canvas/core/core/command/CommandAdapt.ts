...
import {
  convertGoogleDocsToEditorData,
  convertEditorDataToGoogleDocs,
  isGoogleDocsAST
...
import { IPrintOption } from '../../interface/Print'
...
export class CommandAdapt {
  ...
  public exportJson(
    ...
    const value = this.draw.getValue(options)
    const jsonString = JSON.stringify(value, null, 2)
    const blob = new Blob([jsonString], { type: 'application/json' })
    const url = URL.createObjectURL(blob)
    const name = fileName || 'document.json'
    downloadFile(url, name.endsWith('.json') ? name : `${name}.json`)
    URL.revokeObjectURL(url)
    return value
  }

  public async exportJsonAsync(
    fileName?: string,
    options?: IGetValueOption
  ): Promise<IEditorResult> {
    const value = await this.draw.getWorkerManager().getValue(options)
    const jsonString = JSON.stringify(value, null, 2)
    const blob = new Blob([jsonString], { type: 'application/json' })
    const url = URL.createObjectURL(blob)
    const name = fileName || 'document.json'
  ...
  public importJson(
    payload: IEditorResult | IEditorData | string,
    ...
    if (!result) return
    if (isGoogleDocsAST(result)) {
      const gDocData = convertGoogleDocsToEditorData(result)
      this.setValue(gDocData, options)
      return
    }
    if (result.options) {
  ...
  public importGoogleDocs(
    ...
    if (typeof payload === 'string') {
      try {
        ...
        console.error('Invalid Google Docs JSON string', err)
        return
      }
    }
    if (!result) return
    const data = convertGoogleDocsToEditorData(result)
    this.setValue(data, options)
  }

  public getGoogleDocs(title?: string): any {
    ...
    return convertEditorDataToGoogleDocs(editorData, title)
  }

  public exportGoogleDocs(fileName?: string, title?: string): any {
    const gDoc = this.getGoogleDocs(title)
    const jsonString = JSON.stringify(gDoc, null, 2)
    const blob = new Blob([jsonString], { type: 'application/json' })
    const url = URL.createObjectURL(blob)
    const name = fileName || 'google-docs-document.json'
    downloadFile(url, name.endsWith('.json') ? name : `${name}.json`)
    URL.revokeObjectURL(url)
    return gDoc
  }

  public async importDocx(
  ...
  public async getDocx(options?: IExportDocxOption): Promise<Blob> {
    const editorData: IEditorData = this.draw.getValue().data
    const drawOptions = this.draw.getOptions()
    const exportOptions: IExportDocxOption = {
      width: drawOptions.width,
      height: drawOptions.height,
      margins: drawOptions.margins,
      defaultFont: drawOptions.defaultFont,
      defaultSize: drawOptions.defaultSize,
      paperDirection: drawOptions.paperDirection,
      pageNumber: drawOptions.pageNumber,
      ...options
    }
    return convertEditorDataToDocx(editorData, exportOptions)
  }
  ...
  public async exportDocx(
    ...
    const blob = await this.getDocx(options)
    const name = fileName || 'canvas-editor-document.docx'
    if (typeof window !== 'undefined' && typeof document !== 'undefined') {
      const url = URL.createObjectURL(blob)
      downloadFile(url, name.endsWith('.docx') ? name : `${name}.docx`)
  ...
  public getGutter(): { width: number; position: 'left' | 'top' } {
    return {
      width: this.options.gutter ?? 0,
      position: this.options.gutterPosition ?? 'left'
    }
  }

  public setGutter(payload: { width: number; position?: 'left' | 'top' }) {
    this.options.gutter = payload.width
    if (payload.position) {
      this.options.gutterPosition = payload.position
    }
